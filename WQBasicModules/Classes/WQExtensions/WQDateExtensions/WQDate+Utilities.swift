...
extension Date{
    ...
    public func distance(_ ohter:Date, at unit:Calendar.Component , in calendar:Calendar = .current) -> TimeInterval{
...
//        return isSame(Date().dateByAdding(-1, unit: .year, in: calendar)  ,at: .year , in: calendar)
    ...
    public static let commentFlags:Set<Calendar.Component> = [.second,.minute,.hour,.day,.month,.year,.weekOfYear,.weekday,.timeZone]
