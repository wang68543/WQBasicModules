...
//        return decryptedString
    ...
    func encodeDES(_ keyString: String) throws -> Data {
        guard keyString.count == kCCKeySizeDES,
            let iv = keyString.cString(using: .utf8),
            let ASCIIKey = keyString.cString(using: .ascii) else {
            throw NSError(domain: "key initialization failed", code: kCCDecodeError, userInfo: nil)
        }
    ...
    func decodeDES(_ keyString: String) throws -> Data {
        guard keyString.count == kCCKeySizeDES,
            let iv = keyString.cString(using: .utf8),
            let ASCIIKey = keyString.cString(using: .ascii) else {
            throw NSError(domain: "key initialization failed", code: kCCParamError, userInfo: nil)
        }
        let bytes = self.bytes
        let len = self.count
        let buffer = UnsafeMutablePointer<CUnsignedChar>.allocate(capacity: len)
