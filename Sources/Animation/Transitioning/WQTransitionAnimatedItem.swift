...
public protocol WQAnimatedConfigAble {
    ...
    ///   - state: 动画状态
    func config(_ viewController: UIViewController, present state: WQTransitionState)
    @available(iOS 10.0, *)
...
public extension WQAnimatedConfigAble {
    ...
    func setup(_ viewController: UIViewController, present state: WQTransitionState) -> UIViewPropertyAnimator {
        let animator = UIViewPropertyAnimator(duration: 0.25, curve: .easeIn) {
            self.config(viewController, present: state)
        }
        return animator
    }
}
...
public class WQTransitionAnimatedItem<ViewControllerType, Element>: WQAnimatedConfigAble {
    ...
    public func config(_ viewController: UIViewController, present state: WQTransitionState) {
        guard let instance = viewController as? ViewControllerType else {
            return
        }
        switch state {
        case .initial:
            instance[keyPath: self.keyPath] = self.initial
        case .show:
            instance[keyPath: self.keyPath] = self.show
        case .dismiss:
